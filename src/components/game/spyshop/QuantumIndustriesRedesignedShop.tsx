  ...
  // Parallax scroll effect for About Us page background
   useEffect(() => {
    ...
    if (activePage === 'aboutUs' && scroller && bgElement) {
        ...
        bgElement.style.backgroundSize = 'cover, auto 100%'; // Gradient covers, image fills height
        bgElement.style.backgroundPosition = `0% 50%, 0% 50%`;   // Initial: overlay fixed, image at left-center
        ...
        const handleScroll = () => {
            ...
            if (scrollHeight > 0) {
                ...
                // Update X position of the second background layer (the image)
                bgElement.style.backgroundPosition = `0% 50%, ${scrollFraction * 100}% 50%`;
            } else {
                bgElement.style.backgroundPosition = `0% 50%, 0% 50%`; // Reset if not scrollable
            }
        };
