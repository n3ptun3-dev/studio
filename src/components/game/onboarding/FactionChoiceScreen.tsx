...
const factionDetails = {
  Cyphers: {
    ...
    selectedBgClass: "bg-blue-600/40", // Faint background when selected
  ...
  Shadows: {
    ...
    selectedBgClass: "bg-red-600/40", // Faint background when selected
...
export function FactionChoiceScreen({}: FactionChoiceScreenProps) {
  ...
  useEffect(() => {
    ...
    if (selectedFaction === 'Cyphers') {
      ...
      // This screen should default to terminal-green if no faction is actively selected on IT.
      // The global ThemeUpdater will handle overriding this if AppContext.faction changes.
  ...
  }, [selectedFaction, setTheme]); // Corrected: setTheme is stable, currentTheme removed to break loop.
  ...
  const handleConfirmFaction = (factionName: Faction | null) => {
    ...
    setAppContextFaction(factionName); // This will trigger ThemeUpdater
  ...
  const handleProceedAsObserver = () => {
    setSelectedFaction('Observer'); // Local state for this screen
    setAppContextFaction('Observer'); // Global state
    if (currentTheme !== 'terminal-green') { // Ensure this screen is green for observer
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 flex-grow min-h-0 mb-4 sm:mb-6 items-start">
        {(['Cyphers', 'Shadows'] as const).map((factionName) => {
          ...
          const tileClasses = cn(
            ...
            details.borderColorClass, // Always apply team border
