...
const AgentDossierView = () => {
  ...
  const handleFactionChange = () => {
    ...
    // setTheme(newFaction === 'Cyphers' ? 'cyphers' : 'shadows'); // ThemeUpdater handles this
  ...
  const nextLevelXpTargetForDossier = XP_THRESHOLDS[playerStats.level + 1] || (currentLevelXpForDossier + (XP_THRESHOLDS[1] - XP_THRESHOLDS[0] || 100)); // Ensure fallback for max level
...
export function AgentSection({ parallaxOffset }: SectionProps) {
  ...
  const infoAreaRef = useRef<HTMLDivElement>(null); // Ref for the Info Area (stats, timer)
  const thePadRef = useRef<HTMLDivElement>(null); // Ref for "The PAD" main div
  ...
  const handlePowerClick = () => {
    ...
    if (mainScrollContainerRef.current && infoAreaRef.current && thePadRef.current) {
      if (shouldBeUp) {
        // Scroll so "The PAD" (thePadRef) is at the top of mainScrollContainerRef
        // This means scrolling by the height of the "Info Area" (infoAreaRef)
        mainScrollContainerRef.current.scrollTo({
          top: infoAreaRef.current.offsetTop,
          behavior: 'smooth'
        });
      } else {
        // Scroll to the top of the mainScrollContainerRef
  ...
  return (
    <div className="relative flex flex-col p-4 md:p-6 h-full overflow-hidden">
      ...
      <div
        ...
        className="flex-grow flex flex-col overflow-y-auto scrollbar-hide relative"
      >
        {/* "Info Area" (Agent Rank, XP, ELINT, Timer) - No distinct background */}
        <div ref={infoAreaRef} className="text-center pt-2 pb-4 px-2 flex-shrink-0">
        ...
        {/* "The PAD" Container - This gets the background, blur, gloss, rounded corners */}
        <div
          ref={thePadRef}
          className={cn(
            "flex-shrink-0 flex flex-col bg-pad-backing backdrop-blur-sm pad-gloss-effect rounded-t-lg min-h-[160px]", // min-h to ensure it has some height even when screen is hidden
            // Add a subtle border to "The PAD" container
            "border-t border-l border-r border-white/10" // Example: thin white/10 border on top, left, right
          )}
        >
          {/* PAD Button Panel */}
          <div
            className="h-[60px] flex-shrink-0 flex items-center justify-between px-4 border-b border-white/5" // Slightly less prominent border
          >
            {!isPadUp ? (
          ...
          {/* PAD Screen Area - Conditionally rendered/styled based on isPadUp */}
          {isPadUp && (
