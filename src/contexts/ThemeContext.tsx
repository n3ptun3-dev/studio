...
const themeHSLValues: Record<Theme, Record<string, string>> = {
  'terminal-green': {
    ...
    '--hologram-panel-bg-hsl': '130 30% 15%', // Lightness 15% for visibility with opacity
  ...
  'cyphers': {
    ...
    '--hologram-panel-bg-hsl': '210 50% 15%', // Lightness 15%
  ...
  'shadows': {
    ...
    '--hologram-panel-bg-hsl': '0 50% 15%', // Lightness 15%
  ...
  'neutral': {
    ...
    '--hologram-panel-bg-hsl': '220 15% 18%', // Lightness 18%
...
const HSL_VARIABLES_TO_SET_ON_ROOT = [
  ...
  '--hologram-panel-bg-hsl', // Raw HSL for hologram panel base
  // Note: --hologram-glow-color and --hologram-button-text are often direct hsl() values in theme defs
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (themeColorsToSet) {
        ...
        HSL_VARIABLES_TO_SET_ON_ROOT.forEach(variableName => {
          if (themeColorsToSet[variableName]) {
            style.setProperty(variableName, themeColorsToSet[variableName]);
             ...
             if (variableName.endsWith('-debug-color') || variableName.startsWith('--hologram-glow-color-hsl') || variableName.startsWith('--hologram-button-text-hsl')) { // Also clean up direct HSLs if not specifically defined
                style.removeProperty(variableName);
             }
          }
        });

        // Set "effective" color variables that include opacity or are direct hsl() for components
        ...
        // Set effective hologram panel background with opacity
        ...
        // Set direct hologram glow and button text if defined, otherwise CSS classes handle it
        ...
        } else if (themeColorsToSet['--hologram-glow-color']) { // If it's a direct hsl() string
        ...
        } else if (themeColorsToSet['--hologram-button-text']) { // If it's a direct hsl() string
