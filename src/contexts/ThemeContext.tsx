...
const themeHSLValues: Record<Theme, Record<string, string>> = {
  'terminal-green': {
    ...
    '--card-hsl': '130 25% 15%', // Made card slightly lighter for PAD base
  ...
  'cyphers': {
    ...
    '--card-hsl': '210 50% 18%', // Used for PAD base
    ...
    '--hologram-button-text-hsl': 'hsl(0 0% 100%)',   // Direct HSL
    '--cyphers-debug-color': 'blue',
  },
  'shadows': {
    '--background-hsl': '0 60% 8%',
    '--foreground-hsl': '0 0% 90%',
    '--card-hsl': '0 50% 18%', // Used for PAD base
  ...
  'neutral': { 
    ...
    '--card-hsl': '220 15% 18%', // Used for PAD base
    ...
    '--hologram-glow-color-hsl': 'hsl(180 70% 60%)', // Direct HSL
    '--hologram-button-text-hsl': 'hsl(220 10% 95%)', // Direct HSL
...
const HSL_VARIABLES_TO_SET_ON_ROOT = [
  ...
  // Holographic HSL sources (not direct colors)
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (themeColorsToSet) {
        ...
        // Set pre-computed "effective" PAD colors, including opacity, directly on :root
